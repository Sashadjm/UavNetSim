...
def scatter_plot(simulator):
    """Draw a static 3D plot including drones, communication edges, and obstacles."""
    fig = plt.figure()
    ...
    for drone in simulator.drones:
        ax.scatter(drone.coords[0], drone.coords[1], drone.coords[2], c='red', s=30, zorder=5)

    num_drones = len(simulator.drones)
    for i in range(num_drones):
        for j in range(i + 1, num_drones):
            drone1 = simulator.drones[i]
